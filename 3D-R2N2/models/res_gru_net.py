...
from lib.layers import TensorProductLayer, ConvLayer, PoolLayer, Unpool3DLayer, \
    ...
    EltwiseMultiplyLayer, get_trainable_params, FlowLayer, FlowLoss, FlowLayerDummy
...
class ResidualGRUNet(Net):
    ...
    def network_definition(self):
        ...
        flow_shape = (self.batch_size, 3, img_w, img_h)                
        #flow_shape = (self.batch_size, 3, img_w, img_h)        
        # To define weights, define the network structure first
        x = InputLayer(input_shape)
        flow = InputLayer(flow_shape)
        perturbed_x = FlowLayer(x, flow)
        #perturbed_x = FlowLayerDummy(x, flow)        
        conv1a = ConvLayer(perturbed_x, (n_convfilter[0], 7, 7))
        ...
        def recurrence(x_curr, flow_curr, prev_s_tensor, prev_in_gate_tensor):
            # Scan function cannot use compiled function.
            input_ = InputLayer(input_shape, x_curr)

            flow_ = InputLayer(flow_shape, flow_curr)
            perturbed_input_ = FlowLayer(input_, flow_)
            #perturbed_input_ = FlowLayerDummy(input_, flow_)

            #self.flow_loss += FlowLoss(x_curr[1:, :, :, :])
            conv1a_ = ConvLayer(perturbed_input_, (n_convfilter[0], 7, 7), params=conv1a.params)            
        ...
        s_update, _ = theano.scan(recurrence,
            sequences=[self.x, self.flow],  # along with images, feed in the index of the current frame
        ...
        softmax_loss = SoftmaxWithLoss3D(conv11.output, self.mask)

        self.loss = softmax_loss.loss(self.y)
        self.reg_term, self.flow_loss = softmax_loss.flow_loss(self.y, self.flow, self.tau)
        print("SHAPE", self.x.shape)
        total_pixels = self.x.shape[0]*self.x.shape[1]*self.x.shape[2]*self.x.shape[3]*self.x.shape[4]
        self.tv_loss = tensor.sum(tensor.abs_(self.x[:,:,:,1:,:] - self.x[:,:,:,:-1,:])) + tensor.sum(tensor.abs_(self.x[:,:,:,:,1:] - self.x[:,:,:,:,:-1]))
        self.loss = self.loss + cfg.CONST.TV_LOSS_WEIGHT*self.tv_loss
