...
def main(args):
	...
	if args.target == 'all':
		targets = ['car', 'case', 'desk', 'mail', 'tab', 'ones'] 
	else:
		targets = [args.target]
	for target_type in targets:
		if target_type == args.source:
			continue
		solver = Solver(net)                # instantiate a solver

		adv_file_name = args.adv_file
		adv_flow_file_name = args.adv_flow_file
		...
		if target_type != 'ones':
			target_imgs = load_images(target_type)
			target,_ = solver.test_output(target_imgs, flow=flow)
		else:
			target = np.zeros(
				(cfg.CONST.BATCH_SIZE, 32, 2, 32, 32)).astype(theano.config.floatX)

			target[:, :, 1, :, :] = 1
		if not os.path.exists('targets'):
			os.makedirs('targets')
		voxel2obj('targets/' + target_type + '.obj', target[0, :, 1, :, :] > cfg.TEST.VOXEL_THRESH)

		#pdb.set_trace()
		if adv_file_name!='':
			x_adv = np.load(adv_file_name)
		if adv_flow_file_name!='':
			flow = np.load(adv_flow_file_name)

		voxel_prediction, _ = solver.spatial_dag_attack(x_adv, flow, target=target, max_iters=1000, lb=args.lb, tau=args.tau, alpha=args.alpha_flow, alpha_inp=args.alpha_inp, source_img=source_type, target_img=target_type, background_attack=background_attack, weight=args.border_weight, 
...
if __name__ == '__main__':
	...
	parser.add_argument('--adv_file', type=str, default='', help='adversarial file')
	parser.add_argument('--adv_flow_file', type=str, default='', help='adversarial flow file')
	...
	parser.add_argument('--tv_weight', type=float, default=0.1, help='tv loss weight')
	parser.add_argument('--lb', type=float, default=10, help='l2 regularization')
	parser.add_argument('--tau', type=float, default=0.1, help='flow regularization term')
